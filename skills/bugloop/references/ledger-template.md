# Ledger schema

One ledger = one bug = one file, at
`~/.claude/bugloop/<project-slug>/<date>-<slug>.md`. `<project-slug>` is
`basename(project_dir)-<8 char hash of absolute path>`, so two different
repos both named `api` never collide.

Generated by `bugloop.sh init "<desc>"`. `active.json` in the same project
directory points at whichever ledger is "active" — every subcommand acts on
that one by default. To act on a *different*, non-active ledger (a second
in-flight bug, or one the dashboard has open) without disturbing what's
active, pass `--ledger <path>` or set `BUGLOOP_LEDGER`; to make a different
ledger active for the rest of a session, `bugloop.sh switch <id-or-partial>`.

Fields are flat `key: value` lines
(dotted names like `repro.test_cmd` are literal field names, not nesting) so
`get_field`/`set` can find them with a single grep/awk pass. Section headers
(`## repro`, `## baseline`, ...) are for human/model readability only — the
engine does not parse them.

```markdown
# Bug: <one-line description>

state: TRIAGE                      # TRIAGE|REPRO|LOCATE|HYPOTHESIZE|PATCH|
                                    # VERIFY|REVIEW|LANDED|NOT_A_BUG|
                                    # UNREPRODUCED|FLAKY|
                                    # ARCHITECTURE_QUESTION|
                                    # BLOCKED_NEEDS_HUMAN
project_dir: /abs/path
created_at: <ISO 8601 UTC>
test_cmd: <auto-detected or UNKNOWN>
engine_version: <CLAUDE_PLUGIN_ROOT's cache-dir basename, or manual-install>
                                    # which engine build handled this bug —
                                    # stamped once at init, see gates.md note
                                    # on why this exists

## pending
pending.question:                  # verbatim text last asked, empty if none outstanding
pending.context:                    # which trigger asked it (POSSIBLE_NOT_A_BUG, etc.)
pending.asked_at:                   # ISO timestamp
                                    # set by every human-in-the-loop point BEFORE asking,
                                    # cleared right after the answer lands. `resume`/the
                                    # Stop hook surface this automatically when non-empty —
                                    # see references/gates.md's durable-question note.

## repro
repro.test_path:                   # path to the failing test file
repro.test_cmd:                    # command that runs just this test
repro.failing_output:              # literal output, condensed to one line
repro.not_a_bug_evidence:          # set only if NOT_A_BUG: file:line + what it asserts

## baseline
baseline.captured_at:              # ISO timestamp — set by `bugloop.sh baseline`
baseline.failing_tests:            # summary of what already failed pre-fix

## locate
locate.sites:                      # "path:line, path:line, ..." — defs + callers

## hypotheses
- [1] status=testing :: <cause statement> :: <evidence> :: <falsifier>
<!-- Written by `bugloop.sh hypothesis add/refute/confirm` — never hand-edit
these lines. The engine owns this exact format because `gate PATCH` counts
them by regex; a hand-authored line that varies even slightly (spacing,
field order) between runs silently breaks that count. Refuted ones stay,
marked status=refuted, as the record of what not to try again. -->

## patch
patch.files_changed:               # comma-separated paths actually edited

## verify
verify.focused: PASS|FAIL
verify.baseline_diff: CLEAN|<description of new failures>

## review
review.verdict:                    # from cavecrew-reviewer or main thread, incl. any caveats
review.rejection_count:             # bounded-adversarial REVIEW: capped at 2 before
                                    # approve/approve-with-caveats/escalate — see SKILL.md

## landed
landed.committed:                  # yes|no -- set by /bug-land, records what the user chose
landed.commit_sha:                 # if committed -- so LANDED means something specific,
                                    # not just "the ledger reached a terminal state"

## receipts
<!-- `bugloop.sh receipt "<cmd>"` appends a timestamped block here:
### <ISO timestamp> — `<cmd>` — exit <code>
```
<last 40 lines of output>
```
-->
```

## Rules for editing the ledger directly

- `## hypotheses` — engine-owned. Use `bugloop.sh hypothesis add/refute/confirm`,
  never the Edit tool. This is the one section where hand-editing defeats
  the point: `gate PATCH` parses these lines by regex, and only the engine
  writing its own exact format guarantees that parse stays reliable run to
  run.
- `## receipts` — free-form append, via `bugloop.sh receipt "<cmd>"` only
  (not Edit) — it needs the real command output, not a paraphrase.
- Every other field is single-line — use `bugloop.sh set <field> "<value>"`,
  which does an atomic write (temp file + mv), so a crash mid-write never
  corrupts the ledger.
- Never delete a refuted hypothesis or an old receipt. The ledger is a
  record, not a scratchpad — that's what makes it survive `/clear`.
